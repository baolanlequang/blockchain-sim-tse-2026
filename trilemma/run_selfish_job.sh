...
# run_selfish_job.sh` submission: array size = ceil(N_PAIRS / ROWS_PER_TASK).
#   The dataset is now nested_selfish.csv (50,000 pairs), NOT the old 500-row LHS CSV.
...
# submit via:
#   ./submit_chunked_array.sh run_selfish_job.sh 50000 [chunk_size] [sleep_between] [start_offset] [rows_per_task]
#
...
SUBMIT_DIR="$(pwd)"
FULL_CSV=org.palladiosimulator.blockchainsystems.trilemma/nested_selfish.csv
...
# writes). Unlike atosim.jar, selfishmining.jar has no --output-dir flag: it always
# writes to ./result_selfishmining/ relative to CWD, and BlockchainSystemModelLoader
# resolves testmodels/ the same CWD-relative way. So instead of redirecting the
# output path, this task cd's into TASK_WORKDIR with selfishmining.jar and the
...
TASK_WORKDIR="${TMPDIR}/selfish_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
mkdir -p "${TASK_WORKDIR}/result_selfishmining"
...
ln -s "${SUBMIT_DIR}/selfishmining.jar" "${TASK_WORKDIR}/selfishmining.jar"
cd "${TASK_WORKDIR}"
...
cp "${TASK_WORKDIR}/result_selfishmining"/result_*.json "${RESULTS_DIR}/" 2>/dev/null || true
